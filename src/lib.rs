...
//! [`Sender::is_closed`]: tokio::sync::watch::Sender::is_closed
//! [`Sender::closed`]: tokio::sync::watch::Sender::closed
//! [`Sender::subscribe()`]: tokio::sync::watch::Sender::subscribe
...
mod big_notify {
    ...
    impl BigNotify {
        ...
        pub(super) fn notified(&self) -> Notified<'_> {
            let i = tokio::runtime::context::thread_rng_n(8) as usize;
            self.inner[i].notified()
        }
        */
    }
...
mod tests {
    ...
    fn watch_spurious_wakeup() {
        let (send, mut recv) = tokio::sync::watch::channel(0i32);
